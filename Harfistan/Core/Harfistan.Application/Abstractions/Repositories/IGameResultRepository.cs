...
public interface IGameResultRepository : IRepository<GameResult>
{
    ...
    Task<(List<GameResult> Games, int TotalCount)> GetUserHistoryPagedAsync(Guid userId,int pageNumber, int pageSize, CancellationToken cancellationToken = default);
}